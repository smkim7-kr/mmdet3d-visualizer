...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def extract_pts_feat(
        ...
        # Visualization of raw points in bev
        point_bev_vis = vis_bev_points(points)    
        
        voxel_features = self.pts_voxel_encoder(voxel_dict['voxels'],
                                                voxel_dict['num_points'],
                                                voxel_dict['coors'], img_feats,
                                                batch_input_metas) # (7896, 20, 5) -> (7896, 64)
        batch_size = voxel_dict['coors'][-1, 0] + 1
        x = self.pts_middle_encoder(voxel_features, voxel_dict['coors'],
                                    batch_size) # scatter features: (7896, 64) -> (1, 64, 512, 512)
        
        # Visualization of scattered features in bev
        feat_bev_vis = vis_bev_features(x, "multiple", 8)

        x = self.pts_backbone(x) # (1, 64, 512, 512) -> [(1, 64, 256, 256), (1, 128, 128, 128), (1, 256, 64, 64)]
        if self.with_pts_neck: 
            x = self.pts_neck(x) # [...] -> (1, 384, 128, 128)
        # 128 is size of the grid
            
        # Visualization of scattered features in bev
        neck_out_feat_bev_vis = vis_bev_neck_out_features(x, "multiple", 8)
            
        # Merge three visualizations
        # vis_bev_merge(point_bev_vis, feat_bev_vis, neck_out_feat_bev_vis)
    ...
    def extract_feat(self, batch_inputs_dict: dict,
        ...
        """
        voxel_dict = batch_inputs_dict.get('voxels', None) # ['voxels'].shape = (7896, 20, 5)
        ...
        pts_feats = self.extract_pts_feat(
            ...
            points=points,
            img_feats=img_feats,
            batch_input_metas=batch_input_metas) # pts_feats.shape = (1, 384, 128, 128)
    ...
    def predict(self, batch_inputs_dict: Dict[str, Optional[Tensor]],
        ...
        detsamples = self.add_pred_to_datasample(batch_data_samples,
                                                 results_list_3d,
                                                 results_list_2d)
        return detsamples # detsamples[0].keys() = ['pred_instances_3d', 'pred_instances', 'gt_instances', 'gt_instances_3d', 'gt_pts_seg', 'eval_ann_info']


# Visualization functions

def vis_bev_points(points, skip=50, size=0.1):
    """
    Visualize raw point clouds on x and y axis
    skip: number of points / skip will be visualized, use 1 to print all
    size: size of points
    Returns:
        plt: The matplotlib pyplot object for further manipulation
    """
    np_points = points[0].detach().cpu().numpy()
    plt.scatter(np_points[::skip, 0], np_points[::skip, 1], s=size)
    plt.axis('equal')
    plt.savefig('bev_points_visualization.png') 
    
    return plt  

def vis_bev_features(x, mode='first', n=8):
    np_x = x.detach().cpu().numpy() # np_x.shape = (1, 64, 512, 512)
    
    if mode=="first": # visualize first channel
        plt.imshow(np_x[0, 0, :, :])
    elif mode=="random": # visualize random channel
        random_channel = random.randint(0, np_x.shape[1] - 1)
        plt.imshow(np_x[0, random_channel, :, :])
    elif mode=="sum": # visualize channel sum
        sum_channel = np.sum(np_x, axis=1, keepdims=True)
        plt.imshow(sum_channel[0, 0, :, :])
    elif mode=="avg": # visualize channel average
        avg_channel = np.mean(np_x, axis=1, keepdims=True)
        plt.imshow(avg_channel[0, 0, :, :])
    elif mode=="max": # visualize channel maximum
        max_channel = np.max(np_x, axis=1, keepdims=True)
        plt.imshow(max_channel[0, 0, :, :])
    elif mode=="multiple": # visualize multiple channels
        fig, axs = plt.subplots(2, 4, figsize=(20, 10))
        for i in range(8):
            axs[i // 4, i % 4].imshow(np_x[0, i*8, :, :], cmap='hot', interpolation='nearest')
            axs[i // 4, i % 4].set_title(f'Channel {i*8}')
        plt.tight_layout()
    else:
        raise ValueError(f"Invalid mode: {mode}. Please choose from 'first', 'random', 'sum', 'avg', 'max', or 'multiple'.")
    
    plt.savefig('bev_scattered_features_visualization.png')
    return plt  

def vis_bev_neck_out_features(x, mode='first', n=8):
    np_x = x[0][0].detach().cpu().numpy() # np_x.shape = (1, 384, 128, 128)
    sum_channel = np.sum(np_x, axis=0) # sum_channel.shape = (128, 128)
    plt.figure()
    plt.imshow(sum_channel)
    
    plt.savefig('bev_final_features_visualization.png')
    return plt

...
class ImVoteNet(Base3DDetector):
    ...
    def predict(self, batch_inputs_dict: Dict[str, Optional[Tensor]],
        ...
        if points is None:
            ...
            stack_points = torch.stack(points) # stack_points.shape = (1, 20000, 4)
            seeds_3d, seed_3d_features, seed_indices = self.extract_pts_feat(stack_points) # seeds_3d.shape = (1, K=1024, 3) sampliing 1024 points

            vis_seed_pts(seeds_3d)
            ...
            img_features = self.img_mlp(img_features) # img_features.shape = (1, 256, 1024)
            ...
            results_3d = self.pts_bbox_head_joint.predict( # len(labels_3d) = 80, most of their scores_3d is close to 0
                batch_inputs_dict['points'],
                feat_dict,
                batch_data_samples,
                rescale=True)

            num_of_boxes = (results_3d[0]['scores_3d'] > 0.2).sum() # scores above 0.2 is only 5 3d bounding boxes
...
def vis_raw_imgs(imgs):
    imgs = imgs[0].detach().cpu().numpy()
    imgs = imgs.transpose(1, 2, 0)
    imgs += np.array([103.530, 116.280, 123.675])
    plt.imshow(imgs.astype(np.uint8))
    
def vis_raw_pts(pts, skip=10):
    pts = pts[0].detach().cpu().numpy()
    plt.scatter(pts[::skip, 0], pts[::skip, 1], s=1, marker=',')
    plt.show()

def vis_seed_pts(seeds_3d):
    seeds = seeds_3d.detach().cpu().numpy()
    plt.scatter(seeds[:, :, 0], seeds[:, :, 1], s=1, marker=',')
    plt.show()
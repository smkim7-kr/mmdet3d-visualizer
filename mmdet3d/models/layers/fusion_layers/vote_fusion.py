...
from mmdet3d.models.layers.fusion_layers import apply_3d_transformation, bbox_2d_transform, coord_2d_transform
...
class VoteFusion(nn.Module):
    ...
    def forward(self, imgs: List[Tensor], bboxes_2d_rescaled: List[Tensor],
        ...
        for i, data in enumerate(
            ...
            # https://github.com/open-mmlab/mmdetection3d/issues/2981
            ...
            if bbox_num == 0:
                ...
                sem_cue = sem_cue.scatter(-1, bbox_expanded_cls.long(), # sem_cue.shape = (# of seed points = 1024, # of bounding box = 6, # of classes = 10)
                                          bbox_expanded_conf)
                ...
                geo_cue = torch.cat([xz, ray_angle],
                                    dim=-1).view(seed_num, -1, 5) # geo_cue.shape = (1024, # of bounding boxes = 6, 5)
            ...
            txt_cue = txt_cue.unsqueeze(1).expand(-1, 
                                                  self.max_imvote_per_pixel,
                                                  -1).reshape(3, -1) # txt_cue.shape = (3, 3072)

            # append texture cue
            img_feature = torch.cat([two_cues, txt_cue], dim=0)
            img_features.append(img_feature)
            masks.append(mask)
        # total three features = 18

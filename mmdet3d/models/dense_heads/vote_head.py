...
class VoteHead(BaseModule):
    ...
    def predict(self,
        ...
        Returns:
            ...
            scores and labels.
        """
        preds_dict = self(feats_dict) # self maps to forwward
...
def vis_vote_pts(vote_pts):
    votes = vote_pts.detach().cpu().numpy()
    plt.scatter(votes[:, :, 0], votes[:, :, 1], s=1, marker=',')
    plt.show()
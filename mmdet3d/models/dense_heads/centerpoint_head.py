...
class CenterHead(BaseModule):
    ...
    def predict_by_feat(self, preds_dicts: Tuple[List[dict]],
        ...
        """
        rets = []
        for task_id, preds_dict in enumerate(preds_dicts): # len(preds_dicts) = 6
            num_class_with_bg = self.num_classes[task_id]
            batch_size = preds_dict[0]['heatmap'].shape[0]
            batch_heatmap = preds_dict[0]['heatmap'].sigmoid()

            batch_reg = preds_dict[0]['reg'] # (1, 2, 128, 128) => regression value of 128x128 grid
            batch_hei = preds_dict[0]['height'] # (1, 1, 128, 128) => height value of 128x128 grid
            ...
            if 'vel' in preds_dict[0]:
                ...
                batch_vel = None
            temp = self.bbox_coder.decode( # temp[0].keys() = ['bboxes', 'scores', 'labels']
                batch_heatmap,
                batch_rots,
                batch_rotc,
                batch_hei,
        ...
        return ret_list # ret_list[0]['labels_3d'] = value b/w 0 to 9

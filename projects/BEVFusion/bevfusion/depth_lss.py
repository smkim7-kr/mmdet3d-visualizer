...
class DepthLSSTransform(BaseDepthTransform):
    ...
    def get_cam_feats(self, x, d):
        ...
        depth = x[:, :self.D].softmax(dim=1) # depth.shape = (6, 118, 32, 88) 
        x = depth.unsqueeze(1) * x[:, self.D:(self.D + self.C)].unsqueeze(2) # cross product

        x = x.view(B, N, self.C, self.D, fH, fW) # frustum
        x = x.permute(0, 1, 3, 4, 5, 2)
        return x

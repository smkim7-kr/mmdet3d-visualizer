...
model = dict(
    pts_backbone=dict(
        type='PointNet2SASSG', # PointNet++ w/ set abstraction
        in_channels=4,
        num_points=(2048, 1024, 512, 256), # decreasing number of points
        radius=(0.2, 0.4, 0.8, 1.2), # increasing kernel size of points
    ...
    pts_bbox_heads=dict(  
        common=dict(
            ...
            semantic_loss=dict(
                ...
                reduction='sum',
                loss_weight=1.0)),
        joint=dict( # point cloud + camera fusion
